...
void generateExpressions(const vector<int>& nums, int target, vector<string>& result) {
    vector<int> perm = nums;
    do {
        vector<ASTNode*> expressions = generateExpressions(perm, 0, perm.size() - 1);
    ...
    } while (next_permutation(perm.begin(), perm.end()));  // Generate all permutations of nums
}
...
vector<string> findExpressions(const vector<int>& nums, int target) {
    ...
    generateExpressions(nums, target, result);
    return result;
}
